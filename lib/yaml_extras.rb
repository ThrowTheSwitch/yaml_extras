  ...
  def integrate_includes
    ...
    unless @includes_integrated
      ...
      ObjectPathing.find_paths_to_keys(["INCLUDE"], @includes_integrated).each do |l|
        ...
        include_path = ObjectPathing.apply_path(@includes_integrated, l)

        # remove the INCLUDE key. it's not supposed to be in the final result
        target_obj.delete "INCLUDE"

        # if include_path is relative, look at it relative to the current
        # file's location
        unless Pathname.new(include_path).absolute?
          rel_directory = File.dirname(File.expand_path(@file_name))
          include_path = File.join(rel_directory, include_path) 
        end

        # recursively apply YamlExtras in order to deal with any includes that
        # may exist in the included file.
        inc_data = YamlExtras.new(
          include_path,
          @parents.clone << File.expand_path(@file_name)).integrate_includes
